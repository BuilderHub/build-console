...
export default function TeamPage() {
  const [organization, setOrganization] = useState(mockOrganizations[0])
  ...
  return (
    <div className="flex flex-col h-full">
            ...
            {organization.members.map((member) => (
              <div key={member.id} className="flex items-center justify-between py-4 first:pt-0 last:pb-0">
                ...
                <div className="flex items-center gap-3">
                  <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium ${getRoleBadgeColor(member.role)}`}>
      ...
      >
        <form onSubmit={(e) => { e.preventDefault(); handleInviteMember(); }} className="space-y-6">
          <FormField
            label="Email Address"
            name="email"
            type="email"
