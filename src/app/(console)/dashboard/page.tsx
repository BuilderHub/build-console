...
import { mockBuilders, mockBuildLogs, mockOrganizations } from '@/lib/mock-data'
import { 
  Activity, 
  Box, 
  Clock, 
...
export default function DashboardPage() {
  ...
  const activeBuilders = mockBuilders.filter(b => b.status === 'idle' || b.status === 'building')

  const stats = [
    {
      label: 'Active Builders',
      value: activeBuilders.length.toString(),
      change: '+2 this month',
      icon: Box,
      trend: 'up' as const,
    },
    ...
    {
      label: 'Build Minutes',
      value: `${organization.monthlyMinutes}`,
      change: `of ${organization.plan === 'pro' ? '1,000' : 'unlimited'} used`,
      icon: Clock,
  ...
  return (
    <div className="flex flex-col h-full">
      <Header 
        title="Dashboard" 
        subtitle={`Welcome back to ${organization.name}`}
      />
      ...
      <div className="flex-1 p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader 
              title="Recent Builds" 
              subtitle="Latest build activity across all builders"
            />
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      {build.builderName} • {formatDistanceToNow(build.startedAt, { addSuffix: true })}
                    </p>
                  </div>
          ...
          <Card>
            <CardHeader 
              ...
              subtitle={`${activeBuilders.length} builders ready`}
            />
            <div className="space-y-3">
              {activeBuilders.slice(0, 5).map((builder) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      {builder.size} • {builder.region} • {builder.buildCount} builds
                    </p>
                  </div>
                  <div className="flex items-center gap-3 ml-4">
